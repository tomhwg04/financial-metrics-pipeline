...
AS
BEGIN
    -- Initialize pipeline run variables and counters
    ...
    -- Create initial pipeline log entry with STARTED status
    INSERT INTO log.pipeline_run(
        [procedure_name], 
        [status], 
        symbol_filter, 
        trade_date_filter, 
    ...
    -- Execute pipeline processing and track execution metrics
    BEGIN TRY
        -- Count total staging rows and rows within the current processing scope
        ...
        AND (@trade_date IS NULL OR p.trade_date = @trade_date)
        AND (@source IS NULL OR p.source = @source);

        -- Select one best staging row per symbol and trade_date
        -- based on source priority and data freshness
        WITH base_data AS (
            SELECT 
                row_id, 
                batch_id, 
        ...
        -- Apply transactional upsert into core.prices_daily
        BEGIN TRANSACTION;
            -- Update existing core rows when better or newer data is available
            UPDATE c
            SET 
                c.[open] = b.[open],
                c.high = b.high,
            ...
            -- Insert new business keys that do not yet exist in core
        ...
        -- Finalize successful pipeline run and persist execution metrics
        UPDATE l
        SET [status] = 'SUCCESS',
    ...
    -- Roll back open transactions and finalize failed pipeline run
    BEGIN CATCH
        IF @@TRANCOUNT > 0
        BEGIN 
            ROLLBACK;
